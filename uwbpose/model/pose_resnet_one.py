...
class PoseResNet(nn.Module):
    ...
    def __init__(self, block, layers, **kwargs):
        ...
        print("---------------flatten pose net onexone---------------")
        super(PoseResNet, self).__init__()
        # ResNet
        print(INPUT_D, self.inplanes)
        self.conv1 = nn.Sequential(
            #nn.Conv2d(INPUT_D, self.inplanes, kernel_size=3, stride=1, padding=1, bias=False),
            nn.Conv2d(INPUT_D, self.inplanes, kernel_size=4, stride=1, padding=2, bias=False),
            nn.BatchNorm2d(self.inplanes, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True),
            nn.Conv2d(self.inplanes, self.inplanes, kernel_size=4, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(self.inplanes, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True)
        )
