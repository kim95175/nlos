...
class TransPoseR(nn.Module):
    ...
    def __init__(self, block, layers, **kwargs):
        ...
        self.conv1 = nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3, # kernel_size - 7
                               bias=False)
        ...
        d_model = 256 #cfg.MODEL.DIM_MODEL
        dim_feedforward = 1024 #cfg.MODEL.DIM_FEEDFORWARD
        encoder_layers_num = 4 #cfg.MODEL.ENCODER_LAYERS
        n_head = 8 #cfg.MODEL.N_HEAD
        pos_embedding_type = 'sine' #cfg.MODEL.POS_EMBEDDING
        w, h = 32*8, 32*8 #cfg.MODEL.IMAGE_SIZE
        ...
        dummy = torch.zeros(32, 1, 126, 126)
    ...
    def check_dim(self, x):
        print("check dimmension usin dummy = ", x.shape)
...
def get_transpose_r_net(num_layer, **kwargs):
    num_layers = num_layer
    block_class, layers = resnet_spec[num_layers]
    #model = TransPoseR(block_class, layers, cfg, **kwargs)

    model = TransPoseR(block_class, layers, **kwargs)
